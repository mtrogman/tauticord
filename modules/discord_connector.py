    ...
    async def update_voice_channels(self, activity):
        if activity:
            if self.tautulli.voice_channel_settings.get('count', False):
                await self.edit_stream_count_voice_channel(channel_name=self.PlexName + " Streams", count=activity.stream_count)
            if self.tautulli.voice_channel_settings.get('transcodes', False):
                await self.edit_stream_count_voice_channel(channel_name=self.PlexName + " Transcodes",
                                                           count=activity.transcode_count)
            if self.tautulli.voice_channel_settings.get('bandwidth', False):
                await self.edit_bandwidth_voice_channel(channel_name=self.PlexName + " Bandwidth", size=activity.total_bandwidth)
